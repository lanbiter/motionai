...
def _is_connection_error(err: Exception) -> bool:
    ...
    return "connection" in err_name or "connection error" in err_msg
    ...
    try:
        ...
        if llm_provider == "g4f":
            ...
            try:
                ...
                # 部分环境会注入不可用代理，导致 OpenAI 兼容调用直接抛 Connection error。
                # 这里做一次无环境代理兜底，尽量保障 deepseek/openai 等提供方可用。
                if _is_connection_error(first_err) and llm_provider not in {"azure", "modelscope"}:
                    logger.info(
                        f"[{llm_provider}] connection failed with env proxy, retry without env proxy"
                    )
